...
// Function to shuffle chess pieces
function shuffleChessPieces() {
  // Array of chess pieces
  // Shuffle the pieces
  const shuffledPieces = shuffleArray(pieces);

  // Assign shuffled pieces to the squares
  for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
          const id = row * 8 + col + 1;
          const square = document.getElementById(id.toString());
          square.textContent = shuffledPieces[id - 1];
      }
  }
}

// Add click event to the button to shuffle chess pieces
const button = document.querySelector("button");

button.addEventListener("click", shuffleChessPieces);
